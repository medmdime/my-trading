...
# --- my-trading: color indications + win-rate helpers ---
def _pnl_cell(v):
    """Render a PnL number with a green/red indicator for the data tables."""
    try:
        v = float(v)
...
def _win_rate(tp, ts, sl, time_limit, refreshed, failed):
    """Approximate win rate from live close-type counts (TP + trailing = wins).
    For exact per-trade win rate use the Monitor Bots page (reads each executor's PnL)."""
    total = tp + ts + sl + time_limit + refreshed + failed
    if total <= 0:
        return None
    return 100.0 * (tp + ts) / total
            ...
            if bot_status.get("status") == "error":
                ...
                if is_running:
                    ...
                    for controller, inner_dict in performance.items():
                        ...
                        wr = _win_rate(tp, ts, sl, time_limit, refreshed, failed)
                        ...
                        controller_info = {
                            ...
                            "WR %": "—" if wr is None else f"{wr:.0f}%",
                            "Realized PNL ($)": _pnl_cell(round(realized_pnl_quote, 2)),
                            "Unrealized PNL ($)": _pnl_cell(round(unrealized_pnl_quote, 2)),
                            "NET PNL ($)": _pnl_cell(round(global_pnl_quote, 2)),
                    ...
                    with col1:
                        st.metric("🏦 NET PNL", f"${total_global_pnl_quote:.2f}",
                                  delta=round(total_global_pnl_quote, 2))
                    with col2:
                        st.metric("💹 Unrealized PNL", f"${total_unrealized_pnl_quote:.2f}",
                                  delta=round(total_unrealized_pnl_quote, 2))
                    with col3:
                        st.metric("📊 NET PNL (%)", f"{total_global_pnl_pct:.2%}",
                                  delta=f"{total_global_pnl_pct:.2%}")
                    with col4:
